...
const VITE_BASE_URL = 'http://ahli5.test/my-account';
...
{
    $content = file_get_contents(TYPEROCKET_PATH."/wordpress/assets/dist/.vite/manifest.json");
...
function assetUrl(string $entry): string
{
    ...
    return isset($manifest[$entry])
        ? tr_assets_url('dist/') . $manifest[$entry]['file']
        : '';
}
...
function importsUrls(string $entry): array
{
    ...
    if (!empty($manifest[$entry]['imports'])) {
        foreach ($manifest[$entry]['imports'] as $imports) {
            $urls[] = tr_assets_url('dist/') . $manifest[$imports]['file'];
        }
    }
    return $urls;
}
